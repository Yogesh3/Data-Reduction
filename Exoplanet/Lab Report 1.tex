...
\end{equation}
Now that we have the weighted mean of the reference stars, we can get rid of the atmospheric changes in the science images by getting the ratio: $r_{i} = \frac{f_{i}^{sci}}{\mu_{i}^{ref}}$. The error in this ratio is given by:
\begin{equation}
	\sigma_{r_{i}} = r_{i}^{2} \sqrt{\left( \frac{\sigma_{f_{i}^{sci}}}{f_{i}^{sci}}\right) ^{2} + \left( \frac{\sigma_{i}^{ref}}{\mu_{i}^{ref}}\right) ^{2}}
\end{equation}
We now have all the information needed: the time, science flux, weighted mean and the ratio of the two latter. The last step involves normalizing the ratio by the baseline flux of our target star (the flux when the plane is not transiting). The baseline flux was calculated by looking at the clean images taken before and after the transiting occurred, and taking the average flux of those images. The new ratio and its error will then be:
\begin{equation}
	r_{i} \rightarrow \frac{r_{i}}{f_{blf}} \;\;\;\;\;\;\;\;\;\;\; \sigma_{r_{i}} \rightarrow \frac{\sigma_{r_{i}}}{f_{blf}}
\end{equation}
